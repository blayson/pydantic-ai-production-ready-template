...
LITELLM_API_KEY=                                # Virtual API key (optional, can be set via admin panel)
```

**Note:** You can also set `LITELLM_API_KEY` (and other environment variables) directly from the admin panel. See the [Setting LITELLM_API_KEY from Admin Panel](#setting-litellm_api_key-from-admin-panel) section below for detailed instructions.
...
##### Setting LITELLM_API_KEY from Admin Panel
...
1. **Create a Virtual API Key in LiteLLM Dashboard:**
...
2. **Set the API Key via Admin Panel:**
   - Navigate to `http://localhost:8000/admin` and log in with your superuser credentials (create one using `make createsuperuser` if you haven't already)
   - Go to **Env Settings** in the admin panel
   - Add or update the `LITELLM_API_KEY` environment variable:
     - **Key**: `LITELLM_API_KEY`
     - **Value**: Paste the virtual API key you created from the LiteLLM dashboard
...
- [deyaa1251](https://github.com/deyaa1251)
